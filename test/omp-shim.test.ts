...
describe("omp shim spawn adapter lifecycle", () => {
  ...
  test("a child exit clears the settle timer, settles the link, and the next payload spawns", async () => {
    ...
    const { fire } = makeHarness(createSpawnPort(fake.spawnFn, timer.factory));
    fire(TOOL_EVENTS.end, { toolName: "bash" });
    fire("session_stop");
    ...
    expect(fake.children[0]?.json).toBe(
      JSON.stringify({ hook_event_name: "PostToolUse", session_id: "omp-s1", tool_name: "bash" }),
    );
    ...
    expect(fake.children[1]?.json).toBe(JSON.stringify({ hook_event_name: "Stop", session_id: "omp-s1" }));
  ...
  test("a synchronously throwing spawn settles the link fail-soft and the queue continues", async () => {
    ...
    const { fire } = makeHarness(createSpawnPort(spawnFn, makeTimerFactory().factory));
    fire("session_stop");
    fire("input", { source: "interactive" });
    expect(calls).toBe(1);
    await drain();
  ...
  test("a hung helper is SIGKILLed by the settle timeout and the queue releases", async () => {
    ...
    const { fire } = makeHarness(createSpawnPort(fake.spawnFn, timer.factory), 10_000);
    fire(TOOL_EVENTS.end, { toolName: "bash" });
    fire("session_stop");
    expect(fake.children).toHaveLength(1);
