...
Membership is also attention-scoped: a tile exists if and only if the session is active (working, waiting, or error) or unread — idle with an unviewed result. Unread is a per-session ledger (`unread_since`, added in schema v7; the current schema is v8, which only repairs pre-merge v7 databases missing the `model` column). A turn ending — a Stop that settles to idle, or StopFailure — stamps it, because a result landed; only an explicit view clears it: a tile press acks through the daemon (`sessions ack`, the plugin's sole write path), the Paseo overlay reports the agent viewed in Paseo, or a `SessionStart` reuses the session. Prompting again does not mark the earlier result read. A read-and-idle row persists in the registry — subject to the ordinary prune — but is not projected onto the grid, so on the grid idle implies unread. No separate dismiss action exists: viewing the result retires the tile.
