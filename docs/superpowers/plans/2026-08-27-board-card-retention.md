...
- Produces (exact signatures later tasks rely on):
  ...
  - `const paseoSubtreeIdentities = (db: Database, provider: Provider, sessionId: string): Array<{ provider: Provider; sessionId: string }>` — module-private; the seed identity plus its resolved Paseo descendants (stack DFS over `resolvePaseoParentLinks` — traversal order is insignificant: every row's update/delete is independent). Task 3 and Task 5 reuse it.
