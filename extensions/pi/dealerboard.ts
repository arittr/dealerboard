...
/**
 * Reports pi session lifecycle to the dealerboard daemon.
 *
 ...
 * every handler is fail-soft, and helper spawns are detached but serialized
 * through a FIFO queue — each spawn starts only after the previous helper
 * exits, so wire order always matches emission order (unserialized spawns
 * milliseconds apart were live-observed reaching the registry out of order).
 */
...
export const createExtension = (
  ...
  // observed on an Escape'd tool call, where the PostToolUse write landed
